// src/components/pets/DeletePetDialog.jsx
...
import {
  ...
  VaccineRecord,
  PetNote,
  // IMPORTANT: use the exported name "DoseLog" (alias as MedDoseLog for clarity)
  DoseLog as MedDoseLog,
  User,
} from "@/api/entities";
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (medsToDelete.length > 0)
        await Promise.all(medsToDelete.map((i) => Medication.delete(i.id)));
      ...
      if (vaccinesToDelete.length > 0)
        await Promise.all(vaccinesToDelete.map((i) => VaccineRecord.delete(i.id)));
      ...
      if (vetVisitsToDelete.length > 0)
        await Promise.all(vetVisitsToDelete.map((i) => VetVisit.delete(i.id)));
      console.log(`Deleted ${vetVisitsToDelete.length} vet visits`);

      const groomingToDelete = await Grooming.filter(relatedDataFilters);
      if (groomingToDelete.length > 0)
        await Promise.all(groomingToDelete.map((i) => Grooming.delete(i.id)));
      ...
      if (feedingsToDelete.length > 0)
        await Promise.all(
          feedingsToDelete.map((i) => FeedingSchedule.delete(i.id))
        );
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          <Alert className="border-red-200 bg-red-50">
            ...
            <AlertDescription className="text-red-800">
              ...
              related data including: medications, vet visits, grooming records,
              feeding schedules, vaccine records, notes, tasks, and photos.
            </AlertDescription>
          </Alert>
