...
/** Support comma-separated times in time_of_day, e.g. "08:00, 14:00, 22:00" */
...
export default function Medications() {
      ...
      for (const m of meds) {
        try {
          ...
          setHistoryError((prev) => ({
            ...prev,
            [m.id]: "History hidden by security policy. (Owner only)",
          }));
        }
      }
    })();
  ...
  function openAddMeds() {
    ...
    requestAnimationFrame(() => {
      document
        .querySelector("[data-medication-form]")
        ?.scrollIntoView({ behavior: "smooth" });
    });
  }
  function openEditMeds() {
    // still using unified add/edit form; scroll into view
    openAddMeds();
  }
  ...
  function openLogDose(medId) {
    ...
    requestAnimationFrame(() => {
      window.dispatchEvent(
        new CustomEvent("dose-intent", { detail: { petId, medicationId: medId } })
      );
      document
        .querySelector("[data-doselog-form]")
        ?.scrollIntoView({ behavior: "smooth" });
    });
  }
  ...
  async function refreshLogs(medicationId) {
    try {
      ...
      setHasLogs((prev) => ({
        ...prev,
        [medicationId]: (list?.length ?? 0) > 0,
      }));
      setHistoryError((prev) => ({ ...prev, [medicationId]: null }));
    } catch {
      setHistoryError((prev) => ({
        ...prev,
        [medicationId]: "History hidden by security policy. (Owner only)",
      }));
    }
  }
    ...
    if (!logsByMed[medId]) {
      (async () => {
        try {
          ...
          setHasLogs((prev) => ({
            ...prev,
            [medId]: (list?.length ?? 0) > 0,
          }));
        } catch {
          setHistoryError((prev) => ({
            ...prev,
            [medId]: "History hidden by security policy. (Owner only)",
          }));
        }
      })();
    }
  ...
  async function deleteMedication(medicationId) {
    const ok = window.confirm(
      "Delete this medication and all related history? This cannot be undone."
    );
    if (!ok) return;
    try {
      await Meds.remove(medicationId);
      await reloadMeds(petId);
    } catch (e) {
      console.error(e);
    ...
    <div className="page" style={{ paddingTop: 8 }}>
      ...
      <div className="card" style={{ paddingTop: 8 }}>
        {loading ? (
          ...
          <>
            {/* Always-available Add button so you can have multiple meds per pet */}
            <div
              style={{
                display: "flex",
                    ...
                    >
                      ...
                      <div>
                        <div style={{ fontSize: 14 }}>
                          ...
                          {m.time_of_day
                            ? ` @ ${formatTimes(m.time_of_day)}`
                            : ""}
                        </div>
                      </div>
                    </div>

                    {/* Buttons row: Log / Edit / Delete */}
                    <div
                      style={{
                        display: "grid",
